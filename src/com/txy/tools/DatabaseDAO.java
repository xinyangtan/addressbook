...
@SuppressLint("NewApi") public class DatabaseDAO{
	 ...
	 * arg1表示从第几行（行数从零开始）�?始，arg2表示查询几行数据.*/
	@SuppressLint("NewApi") public Map<String,String> queryNumber(String prefix, String center){
		if (center.isEmpty() || !isTableExists("number_" + prefix))
			return null;
		
		int num = Integer.parseInt(center) - 1;
		String sql1 = "select city_id from number_" + prefix + " limit " + num + ",1";//空格不能�? 
	...
	/**查询�?有可能的城市�?*/
	@SuppressLint("NewApi") public Cursor getPossibleCities(String city){
		if (city.isEmpty())
			return null;
	...
	@SuppressLint("NewApi") public Cursor getPossibleCountry(String country){
		if (country.isEmpty())
			return null;
	...
	/**查询城市区号�?*/
	@SuppressLint("NewApi") public Map<String,String> queryCity(String city){
		if (city.isEmpty())
			return null;
